...
# 1. 语法检查（所有 py 文件能编译）
echo "--- 语法检查 ---"
python -m py_compile app/*.py app/agent/*.py app/tools/*.py app/services/*.py app/store/*.py 2>/dev/null && echo "✅ 语法 OK" || { echo "❌ 语法错误"; FAIL=1; }

# 2. 测试（有测试文件就跑）
echo "--- 测试 ---"
if [ -d tests ] && ls tests/test_*.py >/dev/null 2>&1; then
  python -m pytest tests -q 2>/dev/null && echo "✅ 测试全绿" || { echo "❌ 测试失败"; FAIL=1; }
else
